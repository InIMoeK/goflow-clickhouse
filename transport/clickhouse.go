...
_, err = dbConn.Exec(fmt.Sprintf(`CREATE DATABASE IF NOT EXISTS %s`, *ClickHouseDatabase))
...
_, err = dbConn.Exec(fmt.Sprintf(`
  CREATE TABLE IF NOT EXISTS %s.flows_raw (
...
stmt, err := tx.Prepare(fmt.Sprintf(`INSERT INTO %s.flows_raw(TimeReceived, 
    TimeFlowStart,TimeFlowEnd,Bytes,Etype,Packets,SrcAddr,DstAddr,SrcPort,
    DstPort,Proto,SrcMac,DstMac,SrcAS,DstAS,SequenceNum,SamplingRate,SrcVlan,DstVlan,VlanId,FlowType) 
